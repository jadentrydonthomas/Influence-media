...
await p.click('[data-screen="ops"]'); await p.waitForTimeout(900);
await p.screenshot({path:path.join(root,'test','screen-ops.png'), fullPage:true});
const h=await p.evaluate(()=>document.querySelector('#ops').scrollHeight);
console.log('ops height', h, '| JS errors:', errs.length?errs:'none');
await b.close();
