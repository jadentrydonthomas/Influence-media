...
await p.click('[data-screen="customers"]'); await p.waitForTimeout(900);
await p.screenshot({path:path.join(root,'test','screen-customers.png'), fullPage:true});
console.log('height', await p.evaluate(()=>document.querySelector('#customers').scrollHeight), '| errors:', errs.length?errs:'none');
console.log('kpis:', await p.$eval('#custKpis', n=>n.innerText.replace(/\s+/g,' ').slice(0,220)));
await b.close();
